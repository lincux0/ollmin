...
- 支持 PDF、DOCX、XLS、XLSX 本地文件附件：不用模型完成文本/表格值解析，再将有限的相关片段提供给当前请求；
- 设置新会话默认模型和模型别名；
...
在输入框点击“+ 文件”，一次最多添加三个本地 PDF、DOCX、XLS 或 XLSX。Ollmin 在本地解析支持的文件：PDF/DOCX 会拆为文本片段，Excel 会转换为工作表表头与非空行。发送问题时，客户端会按问题选择有限的相关片段，而不是把整份文件直接塞进提示词。

- 4K / 8K / 16K 上下文下，附件参考内容分别限制在约 3K / 6K / 9K 字符；
- 原始文件不会上传，也不会被复制到应用数据目录；
- 扫描件、图片型 PDF 不做 OCR；旧版 Word `.doc` 需要先另存为 `.docx`；
- 附件只归属于发送它的那条用户消息，后续消息不会自动重复发送。
...
- 会话、消息和设置默认只保存在本机；
- 为了在重开会话后恢复附件，解析出的附件片段和元数据会以明文写入同一个本机 SQLite 数据库；不会保存原始文件或源文件路径；
...
支持 PDF、DOCX、XLS 和 XLSX。一次最多添加三个本地文件，单文件不超过 20 MiB。没有文本层的 PDF 需要先 OCR 才能用于对话；旧版 Word `.doc` 会被明确拒绝，因为首个版本只解析 DOCX。
